...
class NetConfig:
    ...
    join: bool = True
    join_gap: float = 50.0     # longest gap joined (px). Raised to 160 on the strength
                               # of a surrogate control that CANNOT SEE this failure -
                               # the surrogate holds no vessel fragments, so invention by
                               # joining two real ones has nothing to join there and the
                               # false-alarm rate does not move. Measured on the real
                               # frame instead, the centreline those joins added sits at
                               # median absorbance 0.039 against 0.082 under the lines
                               # that were already there, and 30 % of it is no darker than
                               # background. Reverted.
